            ...
            <div className="p-4 md:p-8 bg-slate-50 min-h-screen space-y-8">
                    ...
                    <div className="bg-emerald-900 text-emerald-50 p-6 rounded-[2rem] shadow-xl animate-in fade-in slide-in-from-top-4 duration-500">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="font-black flex items-center gap-2 underline decoration-emerald-400">
                                <Zap size={20} className="text-emerald-400" /> AI Financial Analysis
                            </h3>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm italic">
                            <div className="bg-emerald-800/50 p-4 rounded-xl border border-emerald-700">
                                "Your spending on <strong>Expenses</strong> is {data.totalIncome > 0 ? ((data.totalExpenses / data.totalIncome) * 100).toFixed(1) : 0}% of your income."
                            </div>
                            <div className="bg-emerald-800/50 p-4 rounded-xl border border-emerald-700">
                                "You have <strong>{data.recentTransactions.length}</strong> transactions. Largest: ₹ {Math.max(...data.recentTransactions.map(t => t.amount), 0).toLocaleString()}."
                            </div>
                        </div>
                    </div>
                )}
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 font-medium italic">Welcome back! Here's what's happening with your money.</p>
                    </div>
                    <div className="flex gap-3">
                        ...
                        >
                            <Zap size={18} /> {showInsights ? "Close Insights" : "Insights"}
                        </button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            ...
                            <p className="text-slate-400 text-xs font-bold uppercase tracking-[0.2em] mb-1">Available Balance</p>
                            <h2 className="text-4xl font-black text-slate-900">₹ {addThousandsSeparator(data.totalBalance)}</h2>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                        ...
                        <p className="text-slate-400 text-xs font-bold uppercase tracking-[0.2em] mb-1">Total Income</p>
                        <h2 className="text-3xl font-black text-emerald-600">₹ {addThousandsSeparator(data.totalIncome)}</h2>
                    </div>
                    ...
                    <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                        ...
                        <p className="text-slate-400 text-xs font-bold uppercase tracking-[0.2em] mb-1">Total Expenses</p>
                        <h2 className="text-3xl font-black text-rose-600">₹ {addThousandsSeparator(data.totalExpenses)}</h2>
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                        <div className="flex justify-between items-center mb-10">
                            <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest flex items-center gap-2">
                                <Clock size={16} className="text-emerald-500" /> Cash Flow Trend
                            </h3>
                        </div>
                        <div style={{ width: '100%', height: 350 }}>
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={[...data.recentTransactions].slice(0, 10).reverse()}>
                                    <defs>
                    ...
                    <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col">
                        ...
                        <div className="space-y-6 overflow-y-auto max-h-[400px] pr-2 custom-scrollbar">
                            {data.recentTransactions.length > 0 ? (
                                data.recentTransactions.slice(0, 10).map((t, idx) => (
                                    <div key={idx} className="flex items-center justify-between group p-2 rounded-2xl transition-all">
                                        <div className="flex items-center gap-4">
                                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-lg ${t.type === 'income' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                                ...
                                <div className="text-center py-20 text-slate-400 italic">No transactions yet</div>
                            )}
                        </div>
